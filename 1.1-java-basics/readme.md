...
1) В глобальных пременных операционной системы.
...
static поля и блоки инициализируются при загрузке класса с помощью classloader.

// TODO

## Порядок вызова конструкторов и блоков (статических и нестатических)

В общем случае порядок вызова блоков и конструктора следующий:

1. Статический блок
2. Не статический блок
3. Конструктор
...
__Неизменяемый объект (immutable)__ - нельзя ничего изменить после создания объекта. Например: `String` и т.д.
