...
## Иммутабельные (неизменяемые) объекты

Иммутабельными объектами наываются объекты которые не могут изменяться с момента создания и до момента уничтожения. Широкое использование иммутабельных объектов признается отличной стратегией для написание простого и надежного кода. Более того, иммутабельные объекты очень удобны для использования в многопоточном приложении. Когда объект не может быть изменен в течении жизни - можно не беспокоиться о проблемах консистентности данных - все иммутабельные объекты в течении своей жизни всегда будут находиться в консистентном состоянии сколько бы потоков их одновременно не использовали.

Часто разработчики опасаются использовать иммутабельные объекты т.к. опасаются что частое копирование данных для создания новых иммутабельных объектов приводит к проблемам производительности, однако зачастую они пероценивают проблемы создания объектов и недооценивают бонусы от их иммутабельности, тем более что внутренние механизмы JVM, такие как GC, намного быстрее справляются с обработкой именно иммутабельных объектов.
...
class DiscSize {
  ...
  public DiscSize(int freeSize, int osSize, int otherSize) {
    updateSizes(int freeSize, int osSize, int otherSize);
  }
  
  public synchronized void updateSizes(int freeSize, int osSize, int otherSize) {
    this.freeSize = freeSize;
    this.osSize = osSize;
    this.otherSize = otherSize;
    this.totalSize = freeSize + osSize + otherSize;
  }
  ...
  public synchronized int getOsSize() {
    return osSize;
  }
...
Объект типа `DiscSize` всегда в валидном состоянии т.к. он изменяет свое состояние только при создании и в `synchronized`-блоке. Одна представьте что поток А выполняет методы `getTotalSize()` и `getOsSize()` для получения данных о диске, но поток B выполнит метод `updateSizes` между получением потоком B данных из `get`-методов. Таким образом поток B получит неконсистентные данные, и потоку B следовало бы полчить данные захватывая монитор примерно таким образом:

```java
synchronized (discSize) {
    int totalSize = discSize.getTotalSize();
    int osSize = discSize.getOsSize();
}
```

Постоянная работа с объектами, захватывая их монитор, - довольно накладно и с точки зрения производительности и с точки зрения написания кода.

### `final`

Ключевое слово `final` позволяет сделать поле неизменяемым.

```java
class Immutable DiscSize {
...
При такой реализации класса можно не беспокоиться о синхронизации или захватах монитора - объект всегда будет находиться в валидном состоянии, а для изменения размеров можно его пересоздать.

## `java.util.concurrent`

### `locks`

Объекты из пакета `java.util.concurrent.locks` позволяют более гибко управлять блокировками. Основные методы это `lock`, `tryLock` и `unlock`. `tryLock()` или `tryLock(long timeout, TimeUnit unit)` позволяют получать результат блокировки незамедлительно (с таймаутом во втором случае) и выполнять альтернативные ветки кода в случае если ресурс заблокирован в другом потоке. Например, в этом случае поток может освободить свои заблокированные ресурсы, что позволяет избежать тупиков.
...
### Пулы и запускаторы (pools and executors)

Для выполнения большого количества однотипных задач в Java предусмотрены специальные сервисы запуска и пулы выполнения задач. Создавать под каждую маленькую задачу поток - накладно, потоки можно переиспользовать. Для этого нам помогают утилиты пакета `java.util.concurrent`.


#### `Executor`

Базовый интерфейс для запуска задач. Вместо создания и запуска потока `(new Thread(r)).start();` можно запустить `Runnable` объект `r` через `e.execute(r);`, где `r` - это объект типа `Executor`. В дальнейших реализациях этого интерфейса будет видно что это не просто сокращение записей, но в том числе и оптимизация, например, в некоторых реализациях этого интерфейса не создаются нвоые потоки при каждом вызове execute(), а в других можно задать таймаут для запуска задачи.


#### `ExecutorService`

`ExecutorService` является дальнейшим развитием интерфейса `Executor` и предоставляет разработчику дополнительные методы, например `submit` который позволяет выполнять задачи типа `Callable` с возвращаемыми значениями. Так же он позволяет отправлять на выполнение целые коллекции задач. Методы типа `submit` возвращают объекты типа `Future` которые позже могут быть обработаны вызываемым потоком для получения результатов выполнения задач.


#### `ScheduledExecutorService`

Это расширение `ExecutorService` которое кроме всего прочего позволяет настраивать отложенный запуск задач, либо запуск задач с заданными интервалами.
